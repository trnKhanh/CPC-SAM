...
class Focal_loss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2, num_classes=3, size_average=True):
        ...
        if isinstance(alpha, list):
            ...
            print(
                f"Focal loss alpha={alpha}, will assign alpha values for each class"
            )
            self.alpha = torch.Tensor(alpha)
        else:
            assert alpha < 1
            print(
                f"Focal loss alpha={alpha}, will shrink the impact in background"
            )
...
def test_single_volume(
    ...
    for i in range(1, classes + 1):
        metric_list.append(
            calculate_metric_percase(prediction == i, label == i, raw_spacing)
        )
...
def test_single_image(
    ...
    for i in range(1, classes + 1):
        metric_list.append(
            calculate_metric_percase_nospacing(prediction == i, label == i)
        )

    return metric_list


def test_single_image_mean(
    image,
    label,
    net,
    classes,
